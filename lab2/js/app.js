...
document.getElementById('myForm').addEventListener('submit', function(event) {
    ...
    let grade = (mark >= 1 && mark <= 3) ? `Mark: ${mark} - Bad` :
                (mark >= 4 && mark <= 6) ? `Mark: ${mark} - Normal` :
                (mark >= 7 && mark <= 9) ? `Mark: ${mark} - Good` :
                (mark >= 10 && mark <= 12) ? `Mark: ${mark} - Awesome` :
                `Mark: ${mark} - Unknown mark`;
